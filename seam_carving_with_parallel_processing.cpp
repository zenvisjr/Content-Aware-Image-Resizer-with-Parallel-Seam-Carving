...
{
    // Parallelizing the outer loop for row-wise computation
    #pragma omp parallel for
    for(int r=0; r<height_r; r++)
        {
            // Parallelizing the inner loop for column-wise computation
            #pragma omp parallel for
...
void dpVertical(int **a, int height_r, int width_c)
{
    for(int r=1; r<height_r; r++)
        {
            // Parallelizing the inner loop for columns (independent calculations for each column)
            #pragma omp parallel for
            for(int c=0; c<width_c;c++)
            {
                if(c==0)
...
void dpHorizontal(int **a, int height_r, int width_c)
{
    ...
    {
        // Parallelizing the inner loop for rows (independent calculations for each row)
        #pragma omp parallel for
        for(int r=0; r<height_r;r++)
        {   
            if(r==0)
...
int main(int argc, char** argv)
{
    ...
    auto verticalDuration = duration_cast<milliseconds>(stopV - startV);
    cout << "Time required for vertical seam removal: " << verticalDuration.count() << " ms" << endl;
    ...
    auto horizontalDuration = duration_cast<milliseconds>(stopH - startH);
    cout << "Time required for horizontal seam removal: " << horizontalDuration.count() << " ms" << endl;

    //Displaying modified image height and width
    cout<<"New image height: "<<height_r<<endl;
    cout<<"New image width: "<<width_c<<endl;
    auto total_time = verticalDuration.count() + horizontalDuration.count();
    cout<<"Total time take: "<<total_time<<" ms"<<endl;
